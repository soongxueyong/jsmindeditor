...
//    当点击节点信息时查询节点信息并展示出来
//song< support edit node and node's links
// 核心优化：获取节点信息时激活编辑按钮

    function note_select_listener(){
        _jm.add_event_listener(function(type, data) {
            // 判断是否为选中事件
            if (type === jsMind.event_type.select && data.evt === 'select_node')  {
                getSelectedNodeInfo(data.node);
            }
        });
    }
    ...
    function getSelectedNodeInfo() {
        ...
        currentNode = _jm.get_selected_node();
...
//        $g('shared_link').style.display = 'none';
    ...
    const action_handlers = {
        ...
        'edit-node-link': open_link_dialog,
        'edit-node-link-edit-button':switchToEditMode,
        'edit-node-link-save-button':saveNodeChanges,
